...
validar = false;
  ...
  verArchivo(url: string, name: string , index: number) {
    ...
    if (this.esPDF(url) || this.esImagen(url)) {
      ...
      const viewerUrl = this.construirUrlVisorOffice(url);
  ...
  getSafeUrl(url: string): SafeResourceUrl {
      ...
      if (lowerUrl.endsWith('.doc') || lowerUrl.endsWith('.docx') ||
        ...
        const officeViewerUrl = this.construirUrlVisorOffice(url);
  ...
  construirUrlVisorOffice(url: string): string {
    return `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURI(url)}`;
  }
